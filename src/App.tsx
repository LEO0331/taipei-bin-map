...
function App() {
  ...
  if (directoryRoute === 'recycling-analytics') {
    return <div className="app-shell"><header className="app-header"><div><p>{language === 'zh' ? '垃圾清運、回收與環境資料' : 'Waste, recycling & environmental data'}</p><h1>{language === 'zh' ? '資源回收統計' : 'Recycling Analytics'}</h1></div><LanguageToggle language={language} onChange={handleLanguageChange} /><a className="module-route-link" href="#/">{language === 'zh' ? '返回設施地圖' : 'Back to amenities map'}</a></header><main><Suspense fallback={<p>Loading recycling analytics…</p>}><RecyclingAnalyticsDashboard language={language}/></Suspense></main></div>;
  }
  ...
  return (
    <div className="app-shell">
      <header className="app-header">
        ...
        <a className="module-route-link" href="#/swimming-pools">{language === 'zh' ? '臺北市游泳池查詢' : 'Taipei Swimming Pool Finder'}</a><a className="module-route-link" href="#/certified-bathhouses">{language === 'zh' ? '衛生認證浴室' : 'Certified Bathhouses'}</a><a className="module-route-link" href="#/recycling-analytics">{language === 'zh' ? '資源回收統計' : 'Recycling Analytics'}</a>
      </header>
